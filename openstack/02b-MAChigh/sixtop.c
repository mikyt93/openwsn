...
port_INLINE void sixtop_sendEB() {
   ...
   if(Beacons<4){
	   openserial_printInfo(COMPONENT_BEACON,ERR_BEACON_NR,
			   (errorparameter_t)Beacons,
			   (errorparameter_t)0);
   }
   else if(Beacons==4){
	   openserial_printError(COMPONENT_BEACON,ERR_BEACON_OVERFLOWN,
	   			   (errorparameter_t)Beacons,
	   			   (errorparameter_t)0);
   }
   else {
	   openserial_printCritical(COMPONENT_BEACON,ERR_BEACON_FULL,
	   			   (errorparameter_t)Beacons,
	   			   (errorparameter_t)0);
   }
   Beacons++;
